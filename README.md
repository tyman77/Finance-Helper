...
- **Everyone else** → the Summit Sales Travel calendar (`data/calendar_index.json`,
  built by `scripts/fetch_calendar_index.py`): the trip purpose + city around the
  departure date → an overhead account, flagged with the event as context.

Match is by **departure date over the stay window**, joined on the traveler's
email (`CLARK/JOHN → jclark@…` = the event creator). No match → the traveler's
usual account, flagged. Both indices are gitignored (contain names/schedules) and
regenerated by the fetch scripts; in production those run on a cron against the
